...
public class RedisUtil {
    ...
    // Redis服务器IP
    private static String IP = PropertiesFileUtil.getInstance("db").get("redis.host");

    // Redis的端口号
    private static int PORT = PropertiesFileUtil.getInstance("db").getInt("redis.port");

    // 访问密码
    private static String PASSWORD = PropertiesFileUtil.getInstance("db").get("redis.password");
    // 可用连接实例的最大数目，默认值为8；
    // 如果赋值为-1，则表示不限制；如果pool已经分配了maxActive个jedis实例，则此时pool的状态为exhausted(耗尽)。
    private static int MAX_ACTIVE = PropertiesFileUtil.getInstance("db").getInt("redis.max_active");

    // 控制一个pool最多有多少个状态为idle(空闲的)的jedis实例，默认值也是8。
    private static int MAX_IDLE = PropertiesFileUtil.getInstance("db").getInt("redis.max_idle");

    // 等待可用连接的最大时间，单位毫秒，默认值为-1，表示永不超时。如果超过等待时间，则直接抛出JedisConnectionException；
    private static int MAX_WAIT = PropertiesFileUtil.getInstance("db").getInt("redis.max_wait");

    // 超时时间
    private static int TIMEOUT = PropertiesFileUtil.getInstance("db").getInt("redis.timeout");
